<?php
  ...
  class MySQLLayer
  {
    ...
    /** Connect to a MySQL database to be able to use the methods below.
      */
    function MySQLLayer($base, $server, $user, $pass)
    {
      $this->mtStart    = $this->getMicroTime();
      $this->nbQueries  = 0;
      $this->lastResult = NULL;
	  $this->base       = $base;
      $this->conn       = mysql_connect($server, $user, $pass) or die('Server connection not possible.');

      mysql_select_db($this->base) or die('Database connection not possible.');
    }
	...
	/** MEMENTO: Extended to create associative array
	  */
	function queryAssoc($query) {
		...
		while($res = $this->fetchNextAssoc($result))
			$arr[] = $res;
	...
	/** MEMENTO: Keep everything MySQL regarding here
	  */
	...
	/** MEMENTO: Extended to fetch associative array
	  */
