...
if ($_SESSION['logged_in'])
{
	...
	if (!in_array($_GET['type'], $allowed_types))
		die('<p>You must specify valid resource type!</p>');

	// this was moved here to enable dynamic upload directories
	define('CONFIG_SYS_DEFAULT_PATH', "../../../../../".$_SESSION['directory']."/assets/".$_GET['type']); //accept relative path only
	define('CONFIG_SYS_ROOT_PATH', "../../../../../".$_SESSION['directory']."/assets/".$_GET['type']);	//accept relative path only
}
else
{
	die('<p>You must be logged in to use file manager!</p>');
}
